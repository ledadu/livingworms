...
var renderGuiWhip = function(gui, whip) {

    if(_.isUndefined(whip)) {
        return;    
    }

    mainGui.remember(whip);
    
    if(!_.isUndefined(whip.nbsubs)) {
        gui.add(whip, 'nbsubs');
    }
    if(!_.isUndefined(whip.attachNum)) {
        gui.add(whip, 'attachNum');
    }
    gui.add(whip, 'linkCount');
    gui.add(whip, 'minLinkLength');
    gui.add(whip, 'maxLinkLength');
    gui.add(whip, 'width');
    gui.add(whip, 'friction');
    gui.add(whip, 'deltaScale');
    gui.add(whip, 'renderBody');
    gui.add(whip, 'renderParticles');
    gui.add(whip, 'renderLinks');
    if(!_.isUndefined(whip.angleRotation)) {
        gui.add(whip, 'angleRotation');
    }
    if(!_.isUndefined(whip.angleStart)) {
    ...
    if (!_.isUndefined(whip.subWhipsDef) && _.isUndefined(whip.subWhipsDef).linkCount){
    	console.log(whip.subWhipsDef);
    	var f_subWhipsDef = gui.addFolder('subWhipsDef');
    	renderGuiWhip(f_subWhipsDef, whip.subWhipsDef);	
    } else {
    	var f_subWhipsDef = gui.addFolder('subWhipsDef');
    	haveSubwhipsController = gui.add(whip, 'haveSubwhips');
    	haveSubwhipsController.onChange(function(haveSubwhips) {
    		if (haveSubwhips){
    			whip.subWhipsDef = {
					nbsubs          : '1',
					linkCount       : '1',
