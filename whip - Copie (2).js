	...
	for ( var i=0, len = this.links.length; i < len; i++ ) {
		...
		if (i > 0){
			...
			if( testDeltaAngle > angleMax && testDeltaAngle > 0 ){			
				...
				this.links[i].updateParticleBWhithAngle(this.links[i].angle + difAngle*0.2 + deltaPi);
			}
			if( testDeltaAngle < -angleMax && testDeltaAngle < 0 ){
				difAngle = testDeltaAngle - angleMax ;
				this.links[i].updateParticleBWhithAngle(this.links[i].angle + difAngle*.2 + deltaPi);
			}
